...
int main()
{
    ...
    unsigned char *data = stbi_load(CONTAINER_IMAGE_PATH.c_str(), &width, &height, &nrChannels, 0);
    if (data)
    {
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    // load image, create texture and generate mipmaps
    data = stbi_load(AWESOMEFACE_IMAGE_PATH.c_str(), &width, &height, &nrChannels, 0);
    if (data)
    {
